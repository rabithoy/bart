...
sudo sed -i 's|^WIPTER_EMAIL=.*|WIPTER_EMAIL=effie1d9f921@gmail.com|' properties.conf
sudo sed -i 's|^WIPTER_PASSWORD=.*|WIPTER_PASSWORD=lsJhSMdUUWD3@|' properties.conf

sudo sed -i "s|^CASTAR_SDK_KEY=.*|CASTAR_SDK_KEY=cskLF1jAysiPEk|" properties.conf
